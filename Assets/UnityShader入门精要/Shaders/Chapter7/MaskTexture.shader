...
Shader "Unity Shaders Book/Chapter 7/MaskTexture"
{
	...
	{	
		...
		_SpecularMaskScale("Specular Mask Scale", float) = 1.0		//控制遮罩影响度的系数
	}
	...
	{	
		Pass
		{
			...
			fixed4 frag(v2f i): SV_Target {
				...
				fixed4 packedNormal = tex2D(_BumpMap, i.uv);	//利用tex2D对法线纹理_BumpMap进行采样。
				...
				fixed specularMask = tex2D(_SpecularMask, i.uv).r * _SpecularMaskScale;	//对遮罩纹理采样, 并对rgb分量选其一相乘。

				//高光反射计算公式： (C_light * M_specular) * pow(saturate(dot(n, h)), M_gloss);
				fixed3 specular = (C_light * M_specular) * pow(saturate(dot(tangentNormal, halfDir)), M_gloss) * specularMask;	
